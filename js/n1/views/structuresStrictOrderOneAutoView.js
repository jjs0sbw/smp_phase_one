...
(function () {
  ...
  N1.Views.StructuresStrictOrderOneAutoView = Backbone.View.extend({
    el: '#n1-app',
    template: _.template($('#structure-strict-one-auto-tmpl').html()),
    initialize: function () {
      var dataOne = Number.parseInt(this.$('#matrix-size-one-auto').val())
    ...
    enterData: function () {
      ...
      */
      ...
      N1.MathLib.GridTools.autoEnterData(rcOne, rcTwo, canvas, context,
        canvas1, context1, canvas2, context2, cellSize)
      // check matrix upperTriangular1 here one command in gridtools
